...
// data.debug - Debug mode. The default value is false
    ...
    //exports
    global.define  = Module.define; //重要
    global.require = seajs.require; //重要
    global.use     = seajs.use; //重要

    svp.define = Module.define ;//兼容旧版
    svp.require = seajs.require;//兼容旧版
    svp.seajs = seajs;
...
(function (global) {
    ...
    var checkCombo = function() {
        ...
        if( svp && svp.isCombo ) {
            c = true;
        }else if( svp && svp.isCombo =='auto' ){
            c = /h5\.itc\.cn|t\.m\.tv\.sohu\.com/i.test(window.location.href);
        }else{
            c = false;
        }
        return c;
    };
    var isCombo  = checkCombo()||false;
    if (isCombo) {
        var comboHash = data.comboHash = {};
        var comboSyntax = ["??", ","];
        var comboMaxLength = 200;
        var comboExcludes;
...
(function(global) {
    ...
    // normal text require("text!http://xx/data.hbs") || require("./data.tpl")
    register({
        name: "text",
        ext: [".tpl",".html"],
